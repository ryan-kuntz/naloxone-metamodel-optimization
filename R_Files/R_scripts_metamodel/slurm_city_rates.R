## This is the code for returning individual city death count

# ORDER OR OPERATIONS:
# 1. Change model so that "naloxone_data" inputs a row with 0 kits, or a row with minimum kits (should still be much less
#     than 50,000)
...
naloxone_data <- read.csv("naloxone_test_distributions.csv")
...
n.reds <- 64*num_cores    # number of redistribution schemes
...
# OUTPUT for analysis
mat.od.death <- matrix(nrow = number_of_parameters * n.reds, ncol = yr_end - yr_int + 1 + 39) # 39 for number of cities/towns
...
process_scenario <- function(i) {
  ...
  results <- matrix(0, nrow = 1, ncol = 42)
  
  # Subset the naloxone distribution for this scenario
  params$NxOEND.matrix <- params$NxOEND.matrix[rownames(params$NxOEND.matrix) %in% years_to_keep, ]
  naloxone_distribution <- as.numeric(naloxone_data[i, -1])
  naloxone_distribution <- matrix(naloxone_distribution, nrow = 1)
  
  # Add naloxone rows for 2024–2026
  new_rows <- data.frame(matrix(rep(naloxone_distribution, 3), nrow = 3, byrow = TRUE))
  rownames(new_rows) <- c("2024", "2025", "2026")
  colnames(new_rows) <- colnames(naloxone_data)[-1]
  params$NxOEND.matrix <- rbind(params$NxOEND.matrix, new_rows)
  
  vparameters.temp <- sim.data.ls[[job_id]] 
  ...
  # Run microsimulation with unique random numbers
  sim_sq <- MicroSim(
    init_ppl, params = vparameters.temp, timesteps, agent_states,
    discount.rate, PT.out = FALSE, strategy = "SQ", seed = sim.seed[[job_id]]
  )
  
  # Summarize results
  subset_matrix <- sim_sq$m.oddeath[((yr_int - yr_start) * 12 + 1):timesteps, ]
  results[1, 1] <- naloxone_distribution[1, 1]   # Kits per city
  results[1, 2] <- job_id                       # Parameter index
  results[1, 3] <- job_id              # Store actual seed for reproducibility
  results[1, 4:42] <- colSums(subset_matrix[, 1:39])
...
first_col_names <- c("Kits_Per_City", "Parameter", "Seed")

# Define names for the remaining 39 columns from naloxone_distribution
naloxone_col_names <- colnames(read.csv("naloxone_test_distributions.csv", nrows = 1))[-1]
...
output_file <- paste0("City_Rate_param_seed_", job_id, ".csv")
...
# Write the Matrix to a CSV file
#write.csv(mat.od.death, file = "Mat.od.death.csv", row.names = TRUE)