## This file takes should take in a Naloxone Kit Distribution, runs it through 50 simulations (the 50 best fit parameters), 
## then returns the average deaths. 

## This can be used to get the projected deaths from the status quote distribution and from metamodel local minima. 
...
# NOTE: Changed from "status_quote_distribution.csv" to tailored_solution_distribution.csv
naloxone_data <- read.csv("status_quo_distribution.csv")
print(naloxone_data)
...
num_cores <- 25  # Assuming n.reds = 7
n.reds <- 4*num_cores    # number of redistribution schemes
...
process_scenario <- function(i) {
  ...
  vparameters.temp <- sim.data.ls[[i]]
  # Assuming you want the same parameters for all iterations
  ...
  sim_sq <- MicroSim(
    ...
    discount.rate, PT.out = FALSE, strategy = "SQ", seed = sim.seed[[i]]
  )
...
clusterExport(cl, varlist = c(
  ...
  "m.nlx.mn.Pharm", "m.oddeath", "m.odnonfatal", "Costs", "cost.matrix", "m.oddeath.hr"
))
...
naloxone_col_names <- colnames(read.csv("tailored_solution_distribution.csv", nrows = 1))[-1]
...
output_file <- paste0("status_quo_distribution_deaths.csv")
