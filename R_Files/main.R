...
###########################################################################################################
# Define different program scenarios for distributing additional 10,000 kits and initialize matrices and arrays for results
sim.seed <- sim.seed[1] # CHANGED FROM 500 to 1!
scenario.name <- c("Status Quo")
...
for (ss in 1:length(sim.seed)){
  ...
  vparameters.temp <- sim.data.ls[[ss]] # Loads a unique parameter set
...
write.xlsx(mx.od.death.1st, file = ("ODdeaths.xlsx"), sheetName = "1st year", row.names = F)
write.xlsx(mx.od.death.2nd, file = ("ODdeaths.xlsx"), sheetName = "2nd year", append = T, row.names = F)
write.xlsx(mx.od.death.last, file = ("ODdeaths.xlsx"), sheetName= "last year", append = T, row.names = F)
write.xlsx(mx.od.death.totl, file = ("ODdeaths.xlsx"), sheetName= "Total", append = T, row.names = F)
